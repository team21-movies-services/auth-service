...
class TestAdminRoles:
    ...
    async def test_view_roles_user(self, api_client: AsyncClient,
                                   admin_auth_token: dict,
                                   auth_user: AuthUser,
                                   roles_with_user: List[AuthRole]):
        response = await api_client.get(f'/api/v1/users/{auth_user.id}/roles', headers={
            'Authorization': admin_auth_token['access_token']
        })
    ...
    async def test_add_roles_to_user(self, api_client: AsyncClient,
                                     admin_auth_token: dict,
                                     auth_user: AuthUser,
                                     roles: List[AuthRole],
                                     role_repository: RoleRepository):
        role_ids = [str(role.id) for role in roles]
        response = await api_client.post(f'/api/v1/users/{auth_user.id}/roles', json=role_ids,
                                         headers={
                                             'Authorization': admin_auth_token['access_token']
                                         })
        assert response.status_code == HTTPStatus.OK
    ...
    @pytest.mark.parametrize("roles_with_user", [3], indirect=True)
    async def test_delete_roles_to_user(self,
        ...
        response = await api_client.request('DELETE', url=f'/api/v1/users/{auth_user.id}/roles',
                                            json=role_ids,
                                            headers={
                                                'Authorization': admin_auth_token['access_token']
                                            })
