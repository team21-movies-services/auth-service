...
class TestCRUDRoles:
    ...
    async def test_view_roles(self,
                              api_client: AsyncClient,
                              admin_auth_token: dict,
                              roles: List[AuthRole]):
        response = await api_client.get('/api/v1/roles/', headers={
    ...
    async def test_create_role(self,
                               ...
                               admin_auth_token: dict,
                               fake_role_form: dict):
        response = await api_client.post('/api/v1/roles/', json=fake_role_form, headers={
            'Authorization': admin_auth_token['access_token']
        })
    ...
    async def test_delete_role(self,
                               ...
                               role: AuthRole,
                               refresh_object):
        response = await api_client.delete(f'/api/v1/roles/{role.id}', headers={
            'Authorization': admin_auth_token['access_token']
        })
        assert response.status_code == HTTPStatus.OK
    ...
    async def test_update_role(self,
        ...
        response = await api_client.patch(f'/api/v1/roles/{role.id}', json=new_fake_role, headers={
            'Authorization': admin_auth_token['access_token']
        })
        assert response.status_code == HTTPStatus.OK
        role = await refresh_object(role)
